...
instance.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('Development mode - skipping auth error handling, passing error through');
      return Promise.reject(error);
    }
    ...
    if (error.response?.status === 401 && !originalRequest._retry && !isAuthRequest) {
      ...
      try {
        ...
        const response = await axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:8000'}/api/mongodb/auth/mongodb/refresh/`, {
    ...
    if (error.response?.status === 401 && !isAuthRequest) {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log('Development mode - not clearing tokens or redirecting on 401');
        return Promise.reject(error);
      }
