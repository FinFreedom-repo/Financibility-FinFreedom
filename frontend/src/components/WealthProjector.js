...
function WealthProjector({ onNavigateToAccount }) {
  ...
  const loadUserData = async () => {
    try {
      ...
      try {
        const settingsResponse = await axios.get("/api/wealth/settings/");
        ...
        console.warn(
          "⚠️ Could not load wealth projection settings:",
          settingsError
        );
      ...
      try {
        const profileResponse = await axios.get("/api/auth/profile/"); // Updated to MongoDB endpoint
        console.log("✅ Profile response:", profileResponse.data);
        if (profileResponse.data) {
          setUserProfile(profileResponse.data);
          if (profileResponse.data.age) {
            console.log(
              "👤 Setting age from profile:",
              profileResponse.data.age
            );
      ...
      try {
        ...
        if (budgetResponse.data && budgetResponse.data.length > 0) {
          ...
          let monthlyIncome =
            parseFloat(budget.income || 0) +
            parseFloat(budget.additional_income || 0);
      ...
      try {
        ...
        console.log(
          "📊 Accounts response length:",
          accountsResponse.data?.length
        );
        ...
        if (accountsResponse.data && Array.isArray(accountsResponse.data)) {
          ...
          const totalAssets = accountsResponse.data.reduce((sum, account) => {
            ...
            console.log(
              `  - Account ${account.name}: $${balance} (raw: ${account.balance})`
            );
            return sum + balance;
          }, 0);
      ...
      try {
        ...
        if (debtsResponse.data && Array.isArray(debtsResponse.data)) {
          ...
          const totalDebts = debtsResponse.data.reduce((sum, debt) => {
            ...
            console.log(
              `  - Debt ${debt.name}: $${balance} (raw: ${debt.balance})`
            );
            return sum + balance;
          }, 0);
  ...
  const calculateProjection = async () => {
    ...
    try {
      setIsLoading(true);
      console.log(
        "🚀 Sending enhanced projection request with data:",
        formData
      );

      const response = await axios.post("/api/wealth/project/", formData);
  ...
  const importFinancials = async () => {
    try {
      ...
      const response = await axios.get("/api/wealth/import-financials/");
      ...
      if (response.data && response.data.financial_data) {
        ...
        setSuccessMessage(
          `Financial data imported successfully! Found ${
            financialData.accounts_count
          } accounts, ${financialData.debts_count} debts, and ${
            financialData.has_budget ? "budget data" : "no budget data"
          }.`
        );
        setShowSuccessSnackbar(true);
  ...
  const chartData = useMemo(() => {
    ...
    const netWorthData = projectionData.map(
      (item) => item.scenario_1 - item.debt_line
    );
    const netWorthInflAdjData = projectionData.map(
      (item) => item.scenario_2 - item.debt_line
    );
    const netWorthCheckingData = projectionData.map(
      (item) => item.scenario_3 - item.debt_line
    );
    const netWorthCheckingInflAdjData = projectionData.map(
      (item) => item.scenario_4 - item.debt_line
    );
  ...
  const getKeyMetrics = () => {
    ...
    const retirementData = projectionData.find(
    ...
    return {
      ...
      debtFreeAge:
        projectionData.find((item) => item.debt <= 0)?.age || "Never",
      millionaireAge:
        projectionData.find((item) => item.net_worth >= 1000000)?.age ||
        "Never",
    };
  };
              ...
              <CustomCard elevation={2}>
                <CardContent>
                  <Box sx={{ display: "flex", alignItems: "center", mb: 3 }}>
                    ...
                    />
                    <Typography variant="h6">Projection Parameters</Typography>
                  </Box>
                      ...
                      <Grid container spacing={2}>
                        ...
                        <Grid item xs={12}>
                          <TextField
                            ...
                            helperText={
                              validationErrors.maxAge || "Maximum 130 years"
                            }
                            InputProps={{
                              endAdornment: (
                                <InputAdornment position="end">
                      ...
                      <Grid container spacing={2}>
                        <Grid item xs={12}>
                          <TextField
                            ...
                            helperText={
                              validationErrors.startWealth ||
                              "Total of all your account balances"
                            }
                            InputProps={{
                              startAdornment: (
                        ...
                        <Grid item xs={12}>
                          <TextField
                            ...
                            helperText={
                              ...
                              "Total of all your outstanding debts"
                            }
                            InputProps={{
                              startAdornment: (
                                <InputAdornment position="start">
                        ...
                        <Grid item xs={12}>
                          <TextField
                            ...
                            helperText={
                              validationErrors.annualContributions ||
                              "Amount you can save/invest annually"
                            }
                            InputProps={{
                              startAdornment: (
                                <InputAdornment position="start">
                        ...
                        <Stack spacing={2}>
                          <Box>
                            <TextField
                              ...
                              helperText={
                                validationErrors.assetInterest ||
                          ...
                          <Box>
                            <TextField
                              ...
                              helperText={
                                validationErrors.debtInterest ||
                                "Average interest rate on your debts"
                              }
                              InputProps={{
                                endAdornment: (
                                  <InputAdornment position="end">
                          ...
                          <Box>
                            <TextField
                              ...
                              helperText={
                                validationErrors.checkingInterest ||
                                "Interest rate on checking/savings accounts"
                              }
                              InputProps={{
                                endAdornment: (
                                  <InputAdornment position="end">
              ...
              <Stack spacing={3}>
                          ...
                          >
                            ...
                            <Box>
                              <Typography
                                ...
                                sx={{ fontWeight: "bold", fontSize: "1.1rem" }}
                              >
                                {keyMetrics.debtFreeAge === "Never"
                                  ? "Never"
                                  : formatAge(keyMetrics.debtFreeAge)}
                              </Typography>
                              <Typography
                                variant="caption"
                                color="textSecondary"
                          ...
                          >
                            ...
                            <Box>
                              <Typography
                                ...
                                sx={{ fontWeight: "bold", fontSize: "1.1rem" }}
                              >
                                {keyMetrics.millionaireAge === "Never"
                                  ? "Never"
                                  : formatAge(keyMetrics.millionaireAge)}
                              </Typography>
                              <Typography
                                variant="caption"
                                color="textSecondary"
                    ...
                    <CardContent sx={{ p: 3 }}>
                      ...
                      >
                        ...
                        <Box>
                          ...
                          >
                            Interactive chart showing your financial growth
                            trajectory
                          </Typography>
                        </Box>
                      </Box>
                      ...
                      <Box
                        sx={{
                          ...
                          "&::after": {
                            ...
                            bottom: 0,
                            background:
                              "radial-gradient(circle at 30% 20%, rgba(76, 175, 80, 0.03) 0%, transparent 50%), radial-gradient(circle at 70% 80%, rgba(33, 150, 243, 0.03) 0%, transparent 50%)",
                            pointerEvents: "none",
                            borderRadius: 2,
                          },
                        }}
                ...
                <CustomCard elevation={2}>
                  <CardContent>
                    ...
                    >
                      The Enhanced Wealth Projector helps you visualize your
                      financial future by modeling how your wealth will grow
                      over time using 5 different scenarios, including debt
                      repayment simulation, based on your current financial
                      situation and assumptions about returns, inflation, and
                      contributions.
                    </Typography>
                    <Typography
                      variant="body2"
                    ...
                    >
                      • <strong style={{ color: "#FF0000" }}>Red Line:</strong>{" "}
                      Investment Growth After Tax - Wₙ₊₁ = Wₙ × (1 + rₐ × (1 -
                      t))
                      <br />•{" "}
                      <strong style={{ color: "#FF8C00" }}>
                        Orange Line:
                      </strong>{" "}
                      Investment Growth After Tax & Inflation - Wₙ₊₁ = Wₙ × (1 +
                      (rₐ × (1 - t)) - i)
                      <br />•{" "}
                      <strong style={{ color: "#00FF00" }}>
                        Green Line:
                      </strong>{" "}
                      Checking Account Growth (No Taxes) - Wₙ₊₁ = Wₙ × (1 + r𝚌)
                      <br />•{" "}
                      <strong style={{ color: "#0000FF" }}>
                        Blue Line:
                      </strong>{" "}
                      Checking Account Growth After Tax - Wₙ₊₁ = Wₙ × (1 + r𝚌 ×
                      (1 - t))
                      <br />•{" "}
                      <strong style={{ color: "#800080" }}>
                        Purple Line:
                      </strong>{" "}
                      Debt Over Time - Shows how your debt changes with interest
                      and payments
                    </Typography>
                    <Typography
                      variant="body2"
                    ...
                    >
                      • <strong>Starting Wealth (W₀):</strong> Total of all your
                      account balances
                      <br />• <strong>Annual Contributions (C):</strong> Amount
                      you can save/invest each year
                      <br />• <strong>Asset Interest Rate (rₐ):</strong>{" "}
                      Expected annual return on your investments
                      <br />• <strong>Checking Interest Rate (r𝚌):</strong>{" "}
                      Interest rate on checking/savings accounts
                      <br />• <strong>Inflation Rate (i):</strong> Expected
                      annual inflation rate
                      <br />• <strong>Tax Rate (t):</strong> Tax rate on
                      investment returns
                      <br />• <strong>Debt Interest Rate (r_d):</strong> Average
                      interest rate on your debts
                      <br />• <strong>Save Settings:</strong> Store your inputs
                      in MongoDB Atlas for future use
                      <br />• <strong>Import Financials:</strong> Auto-fill form
                      with data from your accounts, debts, and budget
                    </Typography>
                  </CardContent>
                </CustomCard>
              </Stack>
