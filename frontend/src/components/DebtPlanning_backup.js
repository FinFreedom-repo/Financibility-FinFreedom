...
const DebtPlanning = () => {
  ...
  const updateTotalDebtFromPayoffPlan = (gridData, payoffPlan) => {
    ...
    // Fill previous month (plan index 0)
    ...
    // Fill current and future months using plan indices (current = plan[1])
    for (let idx = currentMonthIdx; idx < months.length; idx++) {
      const planIdx = (idx - currentMonthIdx) + 1; // current => 1, next => 2, ...
  ...
  const renderGrid = () => {
    ...
    const onCellValueChanged = async (params) => {
      ...
      const getMonthUrl = `${process.env.REACT_APP_API_URL}/api/mongodb/budgets/get-month/`;
        ...
        }, 200); // Slightly longer delay to ensure propagation is complete
