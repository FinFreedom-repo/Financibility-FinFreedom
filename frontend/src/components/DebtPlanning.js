...
const DebtPlanning = () => {
  ...
  const reloadDebtsAndRecalculate = async () => {
    try {
      ...
      console.log(
        "✅ Debts reloaded, recalculation will be triggered automatically"
      );
    } catch (error) {
  ...
  const generateMonths = useCallback(() => {
    ...
    console.log(
      `📅 Generated ${months.length} months (${historicalMonthsShown} historical + 1 current + ${projectionMonths} future)`
    );
    return months;
        ...
        if (
          ...
          budget.additional_income_items.forEach((incomeItem) => {
            ...
            let additionalIncomeRow = gridData.find(
              (row) =>
                row.category === itemName && row.type === "additional_income"
            );
        ...
        [
          ...new Set(
            budgets
              .map((b) =>
                months.findIndex(
                  (m) => m.month === b.month && m.year === b.year
                )
              )
              .filter((i) => i !== -1)
          ),
  ...
  const calculateNetSavingsFromBudget = (budget) => {
    ...
    const additionalIncome = budget.additional_income_items
      ? budget.additional_income_items.reduce(
  ...
  const createSampleBudgetData = async () => {
    ...
    for (let i = 0; i < projectionMonths; i++) {
      ...
      months.push({
        ...
        actualNetSavings:
          calculatedData.find((row) => row.category === "Net Savings")?.[
            `month_${i}`
          ] || 0,
      });
    }
  ...
  const handleRealTimeGridUpdate = useCallback(
    async (monthIdx, category, newValue, months) => {
      console.log(
        `🔄 Handling real-time grid update for ${category} = ${newValue} in month ${monthIdx}`
      );
      ...
      return new Promise((resolve) => {
        setLocalGridData((prev) => {
          ...
          if (
            ...
            console.log(
              `🆕 Creating new Additional Income row for ${category}`
            );
            ...
            additionalIncomeRow[`month_${monthIdx}`] =
            ...
            const monthBudgetsForDebtCalc = editableMonths.map(
              (budget, idx) => {
                ...
                const netSavingsValue =
                  netSavingsRow[`month_${gridColumnIdx}`] || 0;
                return {
                  ...budget,
  ...
  const propagateCurrentMonthChanges = useCallback(
    async (currentMonthIdx, category, newValue, months, updatedGridData) => {
      const currentVal = parseFloat(newValue) || 0;
      console.log(
        `🚀 OPTIMIZED PROPAGATION: ${category} = ${currentVal} to future months`
      );
      console.log(
        `🔍 All grid data categories:`,
        updatedGridData.map((row) => ({
          category: row.category,
          type: row.type,
        }))
      );
      console.log(
        `🔍 Checking if ${category} is Additional Income:`,
        updatedGridData.find(
          (row) => row.category === category && row.type === "additional_income"
        )
      );
      ...
      console.log(
        `🔄 Starting propagation loop for ${category} from month ${
          currentMonthIdx + 1
        } to ${months.length - 1}`
      );
      for (let i = currentMonthIdx + 1; i < months.length; i++) {
        ...
        console.log(
          `🔄 Propagating ${category} = ${currentVal} to month ${i} (${futureMonth.label})`
        );
        setLocalGridData((prev) => {
          const updated = prev.map((row) => {
            if (row.category === category) {
              console.log(
                `✅ Found row for ${category}, updating month_${i} to ${currentVal}`
              );
      ...
      if (changesToSave.length > 0) {
        try {
          console.log(
            `💾 BATCH SAVING: ${changesToSave.length} changes to database`
          );
          const response = await axios.post(
            "/api/budgets/batch-update/",
            {
          ...
          for (const change of changesToSave) {
            try {
              ...
              console.warn(
                `⚠️ Failed to save ${change.category} for ${change.month}/${change.year}:`,
                e?.message
              );
            }
          }
      ...
      console.log(
        `✅ OPTIMIZED PROPAGATION COMPLETE: ${category} = ${currentVal} to ${propagatedCount} future months`
      );
    },
    [lockedCells]
  );
  ...
  const calculateDebtPayoffPlanFrontend = useCallback(
    (netSavingsData, debts, strategyType, months) => {
      console.log(
      ...
      for (
        ...
        const paidPerDebt =
          totalPaidToDebt > 0
            ? (() => {
                const count =
                  debtBalances.filter((d) => d.balance >= 0).length ||
                  debtBalances.length ||
                  1;
                return totalPaidToDebt / count;
              })()
            : 0;
        const interestPerDebt =
          totalInterest > 0
            ? (() => {
                const count =
                  debtBalances.filter((d) => d.balance >= 0).length ||
        ...
        monthPlan.remainingDebt = debtBalances.reduce(
      ...
      console.log(
        "✅ Frontend debt payoff plan calculated with same-month timing"
      );
      return { plan };
    },
    []
  ...
  const triggerImmediateDebtRecalculation = useCallback(async () => {
    ...
    console.log(
      "🔄 Triggering immediate debt payoff recalculation with corrected timing..."
    );
    setDebtCalculationInProgress(true);
    setPlanLoading(true);
    setDebtRecalculationStatus(
      "Recalculating debt payoff timeline with same-month timing..."
    );
    ...
    try {
      ...
      if (freshPayoffPlan) {
        ...
        setLocalGridData((prevData) => {
          ...
          console.log(
            "Grid update counter incremented to:",
            gridUpdateCounter.current
          );
        ...
        setDebtRecalculationStatus(
          "Debt payoff timeline updated with correct timing!"
        );
      }
    } catch (error) {
      console.error("❌ Error during debt recalculation:", error);
      setDebtRecalculationStatus(
        "Debt recalculation failed. Please try again."
      );
    } finally {
  ...
  const handleDebtChange = useCallback(async () => {
    console.log(
      "🔄 Debt change detected - triggering real-time recalculation..."
    );
    ...
    setDebtCalculationInProgress(true);
    setDebtRecalculationStatus(
      "🔄 Updating debt calculations and payoff timeline..."
    );
  ...
  const onCellValueChanged = useCallback(
    async (params) => {
      ...
      if (data.category === "Net Savings" || data.category === "Remaining Debt")
        return;
      ...
      console.log(
        `🔄 REAL-TIME CELL EDIT: ${data.category} = ${newValue} in ${months[colIdx].label}`
      );
      ...
      try {
        ...
        if (months[colIdx].type === "future") {
          console.log(
            `🔒 LOCKING FUTURE MONTH: ${months[colIdx].label} ${data.category} = ${newValue}`
          );
          setLockedCells((prev) => {
            const next = { ...prev };
            ...
            console.log(
              `🔒 Updated locked cells for month ${colIdx}:`,
              Array.from(setForMonth)
            );
            return next;
          });
        }
        ...
        if (
          months[colIdx].type === "current" &&
          data.category !== "Remaining Debt" &&
          data.category !== "Net Savings"
        ) {
          console.log(
            `🚀 Triggering propagation for ${data.category} in current month`
          );
          ...
          console.log(
            `❌ Not propagating ${data.category} - current month: ${
              months[colIdx].type === "current"
            }, category: ${data.category}`
          );
        }
        ...
        setErrorMessage(
          "Failed to update budget data in real-time. Please try again."
        );
        setShowErrorSnackbar(true);
      } finally {
    ...
    [
      ...
      handleRealTimeGridUpdate,
      propagateCurrentMonthChanges,
      triggerImmediateDebtRecalculation,
    ]
  );
    ...
    (gridData) => {
      ...
      for (let idx = 0; idx < months.length; idx++) {
        ...
        gridData.forEach((row) => {
          ...
          if (row.type === "income" || row.type === "additional_income")
            income += monthValue;
  ...
  const calculateDebtPayoffPlanWithResult = async (
    monthBudgets,
    debts,
    strategyType
  ) => {
    ...
    try {
      ...
      const response = await axios.post("/api/debts/planner/", {
        strategy: strategyType,
        months: monthBudgets.length,
        debts: transformedDebts,
        monthly_budget_data: monthlyBudgetData,
      ...
      console.log(
        "✅ Debt payoff plan calculated successfully:",
        response.data
      );
  ...
  const updateTotalDebtFromPayoffPlan = (gridData, payoffPlan) => {
    ...
    console.log(
      "Current debt values before update:",
      Object.keys(debtRow)
        .filter((key) => key.startsWith("month_"))
        .map((key) => `${key}: ${debtRow[key]}`)
    );
    ...
    payoffPlan.plan.forEach((monthData, index) => {
      ...
      const targetIdx =
        typeof monthData.month === "number" ? monthData.month : index;
      console.log(`Processing month ${targetIdx}, monthData:`, monthData);

      // Calculate total remaining debt for this month
      const totalRemainingDebt = hasCleared
        ? 0
        : monthData.debts.reduce((sum, debt) => sum + (debt.balance || 0), 0);
      ...
      console.log(
        `Month ${targetIdx}: Total remaining debt = $${finalValue.toFixed(2)}`
      );
    });

    console.log("✅ Updated grid with debt payoff plan data");
    console.log(
      "Final debt values after update:",
      Object.keys(debtRow)
        .filter((key) => key.startsWith("month_"))
        .map((key) => `${key}: ${debtRow[key]}`)
    );
  ...
  const saveMonthChangesDirectly = async (month, year, category, value) => {
    console.log(
      `💾 Saving ${category} = ${value} for ${month}/${year} to MongoDB...`
    );
    ...
    try {
      ...
      let existingBudget = budgets.find(
      ...
      if (existingBudget) {
        ...
        if (category === "Primary Income") {
          ...
          if (!updateData.additional_income_items)
            updateData.additional_income_items = [];
          const existingItemIndex =
            updateData.additional_income_items.findIndex(
              (item) => item.name === category
            );
          if (existingItemIndex !== -1) {
            updateData.additional_income_items[existingItemIndex].amount =
              parseFloat(value) || 0;
          } else {
          ...
          updateData.savings_items = [
            { name: "Savings", amount: value, type: "savings" },
          ];
        } else {
        ...
        // Update the budget
        await axios.put(
          `/api/budgets/${existingBudget._id}/update/`,
          updateData
        );
        ...
        const currentMonthBackend = backendBudgets.find(
          (b) =>
            b.month === genMonths[currentIdx].month &&
            b.year === genMonths[currentIdx].year
        );
        const currentPrimaryIncome =
          parseFloat(currentMonthBackend?.income || 0) || 0;
        const currentAdditionalIncome =
          parseFloat(currentMonthBackend?.additional_income || 0) || 0;
        const baseBudget = {
          month,
          year,
          income:
            currentPrimaryIncome ||
            Math.max(0, getGridValue("Income") - currentAdditionalIncome),
          ...
          savings_items: getGridValue("Savings")
            ? [
                {
                  name: "Savings",
                  amount: getGridValue("Savings"),
                  type: "savings",
                },
              ]
            : [],
        ...
        if (category === "Primary Income") {
          ...
          // Handle Additional Income items
          if (!baseBudget.additional_income_items)
            baseBudget.additional_income_items = [];
          const existingItemIndex =
            baseBudget.additional_income_items.findIndex(
              (item) => item.name === category
            );
          if (existingItemIndex !== -1) {
            baseBudget.additional_income_items[existingItemIndex].amount =
              parseFloat(value) || 0;
          } else {
        ...
        const createRes = await axios.post("/api/budgets/create/", baseBudget);
      ...
      console.log(
        `✅ Successfully saved ${category} = ${value} for ${month}/${year} to MongoDB`
      );
    } catch (error) {
      console.error(
        `❌ Failed to save ${category} for ${month}/${year}:`,
        error
      );
      throw error;
    }
  ...
  const renderEnhancedGrid = () => {
    ...
    const netSavingsIndex = reorderedGridData.findIndex(
      (row) => row.category === "Net Savings"
    );
    const remainingDebtIndex = reorderedGridData.findIndex(
      (row) => row.category === "Remaining Debt"
    );
    ...
    if (remainingDebtIndex > -1) {
      const remainingDebtRow = reorderedGridData.splice(
        remainingDebtIndex,
        1
      )[0];
    ...
    const columnDefs = [
      {
        ...
        cellClass: (params) => {
          if (params.data.category === "Net Savings")
            return "net-savings-category-cell";
          if (params.data.category === "Remaining Debt")
            return "remaining-debt-category-cell";
          if (params.data.type === "income") return "income-category-cell";
          if (params.data.type === "additional_income")
            return "additional-income-category-cell";
          ...
          return (
            <Typography
              ...
              sx={{
                ...
                fontSize: "0.95rem",
                fontStyle:
                  data.type === "additional_income" ? "italic" : "normal",
              }}
            >
              {data.type === "additional_income"
                ? `+ ${data.category}`
      ...
      columnDefs.push({
        ...
        cellClass: (params) => {
          ...
          if (params.data.category === "Net Savings") {
            ...
            classes.push(
              value >= 0 ? "net-positive-cell" : "net-negative-cell"
            );
      ...
      >
          ...
          <Fade in={true} timeout={300}>
            <Alert
              ...
              sx={{
                mb: 3,
                background:
                  "linear-gradient(135deg, rgba(255, 0, 0, 0.15), rgba(0, 0, 255, 0.1))",
              }}
              action={
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  ...
                  <Typography variant="body2">
                    {updatingCellInfo
                      ? `Updating ${updatingCellInfo.category}...`
                      : "Updating..."}
                  </Typography>
                </Box>
              }
            >
              <strong>Real-Time Update in Progress:</strong> Your changes are
              being applied across all months and debt calculations are updating
              automatically!
            </Alert>
          </Fade>
        )}
            ...
            <Box
              sx={{
                ...
                "@keyframes glow": {
                  ...
                  "100%": {
                    textShadow:
                      "0 0 20px rgba(76, 175, 80, 0.8), 0 0 30px rgba(76, 175, 80, 0.6)",
                  },
                },
              }}
            />
        ...
        {debtRecalculationStatus && (
          <Alert severity="info" sx={{ mb: 3 }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              ...
              <Typography variant="body2">{debtRecalculationStatus}</Typography>
            </Box>
          </Alert>
        )}
        ...
        <Fade in={true} timeout={800}>
          <Alert
            ...
            sx={{ mb: 3 }}
          >
            <strong>Smart Real-Time Editing:</strong> Edit any cell and watch
            your changes automatically propagate to future months while updating
            debt payoff calculations instantly!
          </Alert>
        </Fade>
  ...
  const calculateDebtStatistics = () => {
    ...
    const remainingDebtRow = localGridData.find(
      (row) => row.category === "Remaining Debt"
    );
    console.log("Remaining Debt row:", remainingDebtRow);
    const currentTotalDebt =
      currentMonthIdx !== -1
        ? remainingDebtRow?.[`month_${currentMonthIdx}`] || 0
        : 0;
    ...
    const currentNetSavings =
      currentMonthIdx !== -1
        ? netSavingsRow?.[`month_${currentMonthIdx}`] || 0
        : 0;
    ...
    if (finalTotalDebt <= 0) {
      ...
      if (debtFreeMonthIndex >= 0) {
        ...
        if (currentMonth) {
          ...
          debtFreeDate = new Date(
            currentMonth.year,
            currentMonth.month - 1 + debtFreeMonthIndex,
            1
          );
        }
      }
      ...
      if (currentMonth) {
        ...
        debtFreeDate = new Date(
          currentMonth.year,
          currentMonth.month - 1 + monthsToPayoff - 1,
          1
        );
      }
      ...
      totalInterest =
        ((finalTotalDebt * avgInterestRate) / 100) * (monthsToPayoff / 12);
  ...
  const calculateStrategySavings = () => {
    ...
    if (payoffPlan && payoffPlan.length > 0) {
      ...
      if (highInterestDebts.length > 0 && lowInterestDebts.length > 0) {
        ...
        avalancheSavings = currentTotalInterest * 0.1;
  ...
  const saveDebt = async () => {
    try {
      ...
      if (
        !debtFormData.interest_rate ||
        parseFloat(debtFormData.interest_rate) < 0
      ) {
      ...
      if (!token) {
        setErrorMessage(
          "Please log in to manage debts. Go to the login page and sign in with your credentials."
        );
        setShowErrorSnackbar(true);
        return;
      }
      ...
      if (editingDebt) {
        ...
        const response = await axios.put(
          `/api/debts/${editingDebt.id || editingDebt._id}/update/`,
          debtData,
          {
            headers: {
        ...
        // Create new debt
        const response = await axios.post("/api/debts/create/", debtData, {
          headers: {
            "Content-Type": "application/json",
          },
        });
  ...
  const confirmDeleteDebt = async () => {
    ...
    try {
      ...
      const response = await axios.delete(
        `/api/debts/${debt.id || debt._id}/delete/`
      );
  ...
  const renderDebtPayoffTimeline = () => {
    ...
    const gridData = [
      {
        ...
        ...months.reduce((acc, _, idx) => {
          ...
          let value =
            idx >= currentMonthIdx
              ? displayPayoffPlan.plan[planIdx]?.debts?.reduce(
      ...
      },
      {
        ...
        ...months.reduce((acc, _, idx) => {
          ...
          const totalPaid =
            displayPayoffPlan.plan[planIdx]?.totalPaid ??
            (displayPayoffPlan.plan[planIdx]?.debts?.reduce(
              ...
              0);
          const totalInterest =
            displayPayoffPlan.plan[planIdx]?.totalInterest ??
            (displayPayoffPlan.plan[planIdx]?.debts?.reduce(
              (sum, debt) => sum + (debt.interest || 0),
              0
            ) ||
              0);
      ...
      },
      {
        ...
        ...months.reduce(
          (acc, _, idx) => ({
            ...
            [`month_${idx}`]:
              idx >= currentMonthIdx
                ? displayPayoffPlan.plan[idx - currentMonthIdx]
                    ?.totalInterest ??
                  (displayPayoffPlan.plan[idx - currentMonthIdx]?.debts?.reduce(
                    (sum, debt) => sum + (debt.interest || 0),
                    0
                  ) ||
                    0)
      ...
      sortedDebts.forEach((debt) => {
        const debtRow = {
          ...
          ...months.reduce((acc, _, idx) => {
            ...
            let value =
              idx >= currentMonthIdx
                ? displayPayoffPlan.plan[planIdx]?.debts?.find(
                    (d) => d.name === debt.name
                  )?.balance || 0
                : parseFloat(debt.balance) || 0;
            if (idx > currentMonthIdx) {
    ...
    const columnDefs = [
      {
        ...
        cellClass: (params) => {
          if (params.data.category === "Remaining Debt")
            return "remaining-debt-category-cell";
          if (params.data.category === "Principal Paid Down")
            return "principal-paid-category-cell";
          if (params.data.category === "Interest Paid")
            return "interest-paid-category-cell";
          ...
          return (
            ...
            >
              <span
                style={{
                  fontWeight:
                    data.category === "Remaining Debt" ? "bold" : "600",
                  fontSize:
                    data.category === "Remaining Debt" ? "1rem" : "0.95rem",
                  lineHeight: 1.2,
                  whiteSpace: "pre-line",
                  color: "inherit",
                }}
              >
                {data.category === "Principal Paid Down"
                  ? "Principal\nPaid Down"
                  : data.category}
              </span>
            </div>
    ...
    const debtFreeMonthIndex = debtFreeDate
      ? months.findIndex((m) => {
      ...
      <Box sx={{ width: "100%" }}>
        ...
        <Box sx={{ mb: 2, display: "flex", justifyContent: "flex-start" }}>
          <Button
            variant="outlined"
            onClick={() =>
              setStrategy(strategy === "snowball" ? "avalanche" : "snowball")
            }
            disabled={debtCalculationInProgress}
          >
            Switch to {strategy === "snowball" ? "Avalanche" : "Snowball"}{" "}
            Strategy
          </Button>
        </Box>
            ...
            "& .ag-theme-alpine": {
              ...
              "& .ag-row": {
                ...
                "& .ag-cell.remaining-debt-row.historical-column": {
                  ...
                  background:
                    "linear-gradient(135deg, rgba(255, 193, 7, 0.4) 0%, rgba(255, 152, 0, 0.4) 100%) !important",
                ...
                "& .ag-cell.remaining-debt-row.current-column": {
                  ...
                  background:
                    "linear-gradient(135deg, rgba(255, 193, 7, 0.9) 0%, rgba(255, 152, 0, 0.9) 100%) !important",
                  color: "#000000 !important",
                  fontWeight: "bold !important",
                ...
                "& .ag-cell.remaining-debt-row.future-column": {
                  ...
                  background:
                    "linear-gradient(135deg, rgba(255, 193, 7, 0.7) 0%, rgba(255, 152, 0, 0.7) 100%) !important",
                  color: "#000000 !important",
                  fontWeight: "bold !important",
                ...
                "& .ag-cell.debt-free-month": {
                  ...
                  background:
                    "linear-gradient(135deg, #bbf7d0 0%, #86efac 100%) !important",
              ...
              "& .ag-pinned-left": {
                ...
                "& .remaining-debt-category-cell": {
                  ...
                  background:
                    "linear-gradient(135deg, rgba(255, 193, 7, 0.3) 0%, rgba(255, 152, 0, 0.3) 100%) !important",
        ...
        <Card sx={{ p: 3 }}>
          ...
          >
            ...
            <Typography variant="caption" color="textSecondary">
              ...
              {historicalMonthsShown} past + 1 current + {projectionMonths}{" "}
              future)
            </Typography>
          </Box>
          ...
          {(() => {
            ...
            const monthlyInterest = outstandingDebts.reduce((sum, debt) => {
              const balance = parseFloat(debt.balance || debt.amount) || 0;
              const interestRate =
                parseFloat(debt.interest_rate || debt.rate) || 0;
              return sum + balance * (interestRate / 100 / 12);
            }, 0);

            // Calculate active debts count
            const activeDebts = outstandingDebts.filter(
              (debt) => (parseFloat(debt.balance || debt.amount) || 0) > 0
            ).length;
                ...
                <Grid container spacing={3}>
                  ...
                  <Grid item xs={12} sm={6} md={3}>
                    ...
                    >
                      <Box sx={{ position: "relative", zIndex: 1 }}>
                        <AccountBalanceIcon
                          sx={{ fontSize: "2.5rem", mb: 1, opacity: 0.9 }}
                        />
                        <Typography
                          variant="h4"
                          sx={{
            ...
            >
              <Box sx={{ display: "flex", alignItems: "center" }}>
                ...
                {outstandingDebts && outstandingDebts.length > 0 && (
                  <Chip
                    label={`${outstandingDebts.length} debt${
                      outstandingDebts.length !== 1 ? "s" : ""
                    }`}
                    size="small"
                    sx={{
                      ml: 2,
            ...
            {debtsError ? (
              ...
              >
                <Box sx={{ mb: 2 }}>
                  ...
                  >
                    Please log in to manage your debts. Use the login page to
                  ...
                  <Grid container spacing={3} sx={{ mb: 3 }}>
                      ...
                      >
                        ...
                        >
                          {formatCurrency(
                            outstandingDebts.reduce((sum, debt) => {
                              const balance =
                                parseFloat(debt.balance || debt.amount) || 0;
                              return sum + balance;
                            }, 0)
                          )}
                        </Typography>
                          ...
                          {formatCurrency(
                            outstandingDebts.reduce((sum, debt) => {
                              const balance =
                                parseFloat(debt.balance || debt.amount) || 0;
                              const interestRate =
                                parseFloat(debt.interest_rate || debt.rate) ||
                                0;
                    ...
                    <Grid item xs={12} sm={6} md={3}>
                      <Card
                        sx={{
                          background:
                            "linear-gradient(45deg, #0000ff, #0000cc)",
                          color: "white",
                          textAlign: "center",
                          p: 2,
                          mx: 0.5,
                    ...
                    >
                      <Box sx={{ display: "flex", alignItems: "center" }}>
                        ...
                        <Box>
                          ...
                          >
                            {outstandingDebts.length} active debt
                            {outstandingDebts.length !== 1 ? "s" : ""}
                          </Typography>
                        </Box>
                      </Box>
                    ...
                    (!outstandingDebts || outstandingDebts.length === 0) && (
                      ...
                      >
                        <Box>
                          ...
                          >
                            You're debt-free! Add your first debt to start
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
            ...
            <FormControl fullWidth required>
              ...
              <Select
                ...
                onChange={(e) =>
                  handleDebtFormChange("debt_type", e.target.value)
                }
                label="Debt Type"
                sx={{
                  color: isDarkMode ? "white" : "#2c3e50",
            ...
            <TextField
              ...
              onChange={(e) =>
                handleDebtFormChange("interest_rate", e.target.value)
              }
              fullWidth
              required
            ...
            <TextField
              ...
              onChange={(e) =>
                handleDebtFormChange("payoff_date", e.target.value)
              }
              fullWidth
          ...
          {deleteConfirmDialog.debt && (
            ...
            >
              ...
              >
                ...
                {(
                  parseFloat(
                    deleteConfirmDialog.debt.interest_rate ||
                      deleteConfirmDialog.debt.rate
                  ) || 0
                ).toFixed(2)}
                %
              </Typography>
