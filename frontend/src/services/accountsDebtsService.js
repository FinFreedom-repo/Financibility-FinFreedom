...
class AccountsDebtsService {
    ...
    try {
      ...
      if (error.response) {
        if (error.response.status === 401) {
          ...
          throw new Error(
            "Access denied. You do not have permission to view accounts."
          );
        } else if (error.response.status === 404) {
          throw new Error(
            "Accounts endpoint not found. Please check the API configuration."
          );
        } else {
          const errorMessage =
  ...
  async getDebts() {
    try {
      ...
      const totalDebt = processedDebts.reduce(
      ...
      console.error(
        "❌ Error fetching debts from authenticated endpoint:",
        error
      );
      ...
      if (
        ...
        try {
          const testResponse = await axios.get("/api/debts/test/");
          console.log("✅ Test endpoint success:", testResponse.data);

          // Process test response same way
          const debts = Array.isArray(testResponse.data.debts)
            ? testResponse.data.debts
            : [];
          const processedDebts = debts.map((debt) => {
            const balance = parseFloat(debt.amount || debt.balance) || 0;
            const interestRate = parseFloat(debt.interest_rate) || 0;
      ...
      if (error.response) {
        if (error.response.status === 401) {
          ...
          throw new Error(
            "Access denied. You do not have permission to view debts."
          );
        } else if (error.response.status === 404) {
          throw new Error(
            "Debts endpoint not found. Please check the API configuration."
          );
        } else {
          const errorMessage =
            error.response.data?.detail ||
  ...
  async createAccount(accountData) {
    try {
      ...
      const formattedData = {
        ...
        account_type:
          accountData.accountType || accountData.account_type || "checking",
        balance: parseFloat(accountData.balance),
        interest_rate: parseFloat(
          accountData.interestRate || accountData.interest_rate
        ),
        effective_date:
          accountData.effectiveDate ||
          accountData.effective_date ||
          new Date().toISOString().split("T")[0],
      };

      console.log("Formatted account data:", formattedData);
      const response = await axios.post("/api/accounts/create/", formattedData);
  ...
  async createDebt(debtData) {
    try {
      ...
      const formattedData = {
        ...
        interest_rate: parseFloat(
          debtData.interestRate || debtData.interest_rate
        ),
        effective_date:
          debtData.effectiveDate ||
          debtData.effective_date ||
          new Date().toISOString().split("T")[0],
        payoff_date: debtData.payoffDate || debtData.payoff_date || null,
      };

      console.log(`📤 Creating debt via: /api/debts/create/`);
      console.log("Formatted debt data:", formattedData);
      const response = await axios.post("/api/debts/create/", formattedData);
  ...
  async deleteAccount(accountId) {
    try {
      await axios.delete(`/api/accounts/${accountId}/delete/`);
      return true;
    } catch (error) {
  ...
  async deleteDebt(debtId) {
    try {
      ...
      await axios.delete(`/api/debts/${debtId}/delete/`);
  ...
  async bulkSaveAccountsDebts(accounts, debts) {
    try {
      ...
      for (const debt of debts) {
        try {
          const response = await axios.post("/api/debts/create/", debt);
  ...
  async calculateDebtPayoffPlan(data) {
    try {
      ...
      if (
        !data.strategy ||
        !["snowball", "avalanche"].includes(data.strategy)
      ) {
      ...
      const validatedDebts = data.debts.map((debt) => {
        ...
        if (debt.balance === undefined || debt.balance === null)
          throw new Error(`Balance is required for debt: ${debt.name}`);
        if (debt.rate === undefined || debt.rate === null)
          throw new Error(`Interest rate is required for debt: ${debt.name}`);
      ...
      if (error.response) {
        ...
        if (error.response.status === 401) {
          ...
          throw new Error(
            "Access denied. You do not have permission to calculate debt payoff plans."
          );
        } else if (error.response.status === 404) {
          throw new Error(
            "Debt planner endpoint not found. Please check the API configuration."
          );
        } else if (error.response.status === 400) {
          const errorMessage =
            errorData.error || errorData.detail || "Invalid request data";
          throw new Error(errorMessage);
        } else {
          throw new Error(
            errorData.error ||
              errorData.detail ||
              "Failed to calculate debt payoff plan"
          );
        }
      } else if (error.request) {
        throw new Error(
