#!/usr/bin/env python3
"""
Comprehensive test script to test the complete integration
"""
...
def test_complete_integration():
    ...
    # 1. Test getting all budgets
    print("\n1. Testing GET budgets...")
    try:
    ...
    # 3. Test debt payoff calculation
    print("\n3. Testing debt payoff calculation...")
    try:
        debt_payoff_data = {
            "debts": [
                {
    ...
    # 4. Test getting updated budgets
    print("\n4. Testing GET updated budgets...")
    try:
        response = requests.get('http://localhost:8000/api/mongodb/budgets/test/')
        if response.status_code == 200:
            data = response.json()
            budgets = data.get('budgets', [])
            print(f"✅ Found {len(budgets)} budgets after updates")
            
            # Show the updated budget
            for budget in budgets:
                if budget.get('month') == 10 and budget.get('year') == 2025:
                    print(f"✅ Updated budget for 10/2025:")
