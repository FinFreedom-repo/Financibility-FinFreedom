"""
Wealth Projection Calculators
Calculate wealth projections with multiple scenarios
"""
...
def calculate_wealth_projection(data):
    ...
    checking_interest_rate = float(data.get('checkingInterest', 4)) / 100
    debt_interest_rate = float(data.get('debtInterest', 0)) / 100
    
    # Initialize values
    current_age = int(data['age'])
    max_age = int(data.get('maxAge', 100))
    years_to_project = max_age - current_age
    annual_contribution = float(data['annualContributions'])
    
    # Initialize wealth and debt
    W0 = float(data['startWealth'])
    D0 = float(data.get('debt', 0))
    
    # Store projections
    projections = []
    
    # Add the first data point (year 0)
    projections.append({
        'year': 0,
        'age': current_age,
        'scenario_1': round(W0, 2),
        'scenario_2': round(W0, 2),
        'scenario_3': round(W0, 2),
        'scenario_4': round(W0, 2),
        'debt_line': round(D0, 2),
    ...
    for year in range(1, years_to_project + 1):
        ...
        projections.append({
            ...
            'scenario_1': round(W1, 2),
            'scenario_2': round(W2, 2),
            'scenario_3': round(W3, 2),
            'scenario_4': round(W4, 2),
            'debt_line': round(debt, 2),
            'net_worth': round(net_worth_1, 2),
            'wealth': round(W1, 2),
            'debt': round(debt, 2),
