#!/usr/bin/env python3
"""
Simple HTTP proxy to forward mobile app requests to Django backend
"""
import http.server
import socketserver
...
class ProxyHandler(http.server.BaseHTTPRequestHandler):
    ...
    def proxy_request(self, method):
        try:
            # Forward to Django backend on localhost
...
if __name__ == "__main__":
    ...
    with socketserver.TCPServer(("0.0.0.0", PORT), ProxyHandler) as httpd:
        ...
        print(f"📋 Django backend must be running on localhost:8000")
        httpd.serve_forever()
