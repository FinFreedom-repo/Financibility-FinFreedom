...
class BudgetService {
  ...
  async getBudgets(): Promise<Budget[]> {
    try {
      ...
      const budgets = Array.isArray(responseData?.budgets) ? responseData.budgets : [];
      ...
      return budgets.map((budget: any) => ({
        ...
        savings_items: Array.isArray(budget.savings_items) 
          ...
          : [],
        manually_edited_categories: Array.isArray(budget.manually_edited_categories) 
      ...
      throw new Error(`Failed to fetch budgets: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async getMonthBudget(month: number, year: number): Promise<Budget | null> {
    try {
      ...
      const response = await apiClient.get(`/api/mongodb/budgets/get-month/?month=${month}&year=${year}`);
      ...
      throw new Error(`Failed to fetch month budget: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async createBudget(budgetData: CreateBudgetData): Promise<Budget> {
    try {
      ...
      const response = await apiClient.post('/api/mongodb/budgets/create/', budgetData);
      ...
      return {
        ...
        additional_income: Number(responseData.additional_income) || budgetData.additional_income,
        additional_income_items: Array.isArray(responseData.additional_income_items) 
        ...
        expenses: {
          housing: Number(responseData.expenses?.housing) || budgetData.expenses.housing,
          transportation: Number(responseData.expenses?.transportation) || budgetData.expenses.transportation,
          food: Number(responseData.expenses?.food) || budgetData.expenses.food,
          healthcare: Number(responseData.expenses?.healthcare) || budgetData.expenses.healthcare,
          entertainment: Number(responseData.expenses?.entertainment) || budgetData.expenses.entertainment,
          shopping: Number(responseData.expenses?.shopping) || budgetData.expenses.shopping,
          travel: Number(responseData.expenses?.travel) || budgetData.expenses.travel,
          education: Number(responseData.expenses?.education) || budgetData.expenses.education,
          utilities: Number(responseData.expenses?.utilities) || budgetData.expenses.utilities,
          childcare: Number(responseData.expenses?.childcare) || budgetData.expenses.childcare,
          debt_payments: Number(responseData.expenses?.debt_payments) || budgetData.expenses.debt_payments,
          others: Number(responseData.expenses?.others) || budgetData.expenses.others
        },
        ...
        manually_edited_categories: Array.isArray(responseData.manually_edited_categories) 
  ...
  async updateBudget(budgetId: string, budgetData: CreateBudgetData): Promise<Budget> {
    try {
      const response = await apiClient.put(`/api/mongodb/budgets/${budgetId}/update/`, budgetData);
      const responseData = response.data as any;
  ...
  async saveMonthBudget(budgetData: CreateBudgetData): Promise<Budget> {
    try {
      ...
      const response = await apiClient.post('/api/mongodb/budgets/save-month/', budgetData);
      ...
      return {
        ...
        additional_income: Number(budget.additional_income) || budgetData.additional_income,
        additional_income_items: Array.isArray(budget.additional_income_items) 
          ? budget.additional_income_items.map((item: any) => ({
              name: item.name || '',
              amount: Number(item.amount) || 0
          ...
          : budgetData.additional_income_items,
        expenses: {
          housing: Number(budget.expenses?.housing) || budgetData.expenses.housing,
          transportation: Number(budget.expenses?.transportation) || budgetData.expenses.transportation,
          food: Number(budget.expenses?.food) || budgetData.expenses.food,
          healthcare: Number(budget.expenses?.healthcare) || budgetData.expenses.healthcare,
          entertainment: Number(budget.expenses?.entertainment) || budgetData.expenses.entertainment,
          shopping: Number(budget.expenses?.shopping) || budgetData.expenses.shopping,
          travel: Number(budget.expenses?.travel) || budgetData.expenses.travel,
          education: Number(budget.expenses?.education) || budgetData.expenses.education,
          utilities: Number(budget.expenses?.utilities) || budgetData.expenses.utilities,
          childcare: Number(budget.expenses?.childcare) || budgetData.expenses.childcare,
          debt_payments: Number(budget.expenses?.debt_payments) || budgetData.expenses.debt_payments,
      ...
      throw new Error(`Failed to save month budget: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  calculateBudgetSummary(budget: Budget): BudgetSummary {
    ...
    const totalAdditionalIncome = budget.additional_income_items.reduce((sum, item) => sum + (item.amount || 0), 0);
    const totalIncome = primaryIncome + totalAdditionalIncome;
    
    const totalFixedExpenses = Object.values(budget.expenses).reduce((sum, value) => sum + (value || 0), 0);
    const totalAdditionalExpenses = budget.additional_items.reduce((sum, item) => sum + (item.amount || 0), 0);
    const totalExpenses = totalFixedExpenses + totalAdditionalExpenses;
    
    const totalSavings = budget.savings_items.reduce((sum, item) => sum + (item.amount || 0), 0);
  ...
  async testSaveMonthBudget(budgetData: CreateBudgetData): Promise<Budget> {
    try {
      ...
      const response = await apiClient.post('/api/mongodb/budgets/save-month-test/', budgetData);
      ...
      throw new Error(`Failed to test save month budget: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  getExpenseCategories() {
    return {
      ...
      transportation: { label: 'Transportation', icon: 'car', color: '#4ECDC4' },
      ...
      others: { label: 'Miscellaneous', icon: 'ellipsis-horizontal', color: '#82E0AA' }
    };
  }
  ...
  getSavingsOptions() {
    return [
      { name: 'Emergency Fund', amount: 0, icon: 'shield-checkmark', color: '#FF6B6B' },
      { name: 'Retirement (401k)', amount: 0, icon: 'time', color: '#4ECDC4' },
      { name: 'Retirement (IRA)', amount: 0, icon: 'time', color: '#45B7D1' },
      { name: 'Investment Account', amount: 0, icon: 'trending-up', color: '#96CEB4' },
      ...
      { name: 'Children Education Fund', amount: 0, icon: 'school', color: '#F7DC6F' },
      ...
      { name: 'Others', amount: 0, icon: 'ellipsis-horizontal', color: '#82E0AA' }
    ];
  }
}
