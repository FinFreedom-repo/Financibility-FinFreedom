...
class AccountsDebtsService {
  ...
  async getAccountsDebtsSummary(): Promise<AccountsDebtsSummary> {
    try {
      ...
      const totalAccountBalance = accounts.reduce((sum: number, acc: any) => sum + (parseFloat(acc.balance) || 0), 0);
      const totalDebtBalance = debts.reduce((sum: number, debt: any) => sum + (parseFloat(debt.balance || debt.amount) || 0), 0);
  ...
  async getDebts(): Promise<Debt[]> {
    try {
      ...
      const responseData = response.data as any;
      const debts = Array.isArray(responseData?.debts) ? responseData.debts : [];
  ...
  async createAccount(accountData: CreateAccountData): Promise<Account> {
    try {
      ...
      const response = await apiClient.post('/api/mongodb/accounts/create/', formattedData);
      const responseData = response.data as any;
      ...
      return {
        ...
        interest_rate: parseFloat(responseData.interest_rate) || accountData.interestRate,
        effective_date: responseData.effective_date || accountData.effectiveDate,
  ...
  async createDebt(debtData: CreateDebtData): Promise<Debt> {
    try {
      ...
      const response = await apiClient.post('/api/mongodb/debts/create/', formattedData);
      const responseData = response.data as any;
      ...
      return {
        ...
        balance: parseFloat(responseData.balance || responseData.amount) || debtData.balance,
        amount: parseFloat(responseData.amount || responseData.balance) || debtData.balance,
        interest_rate: parseFloat(responseData.interest_rate) || debtData.interestRate,
  ...
  // Update an account
  async updateAccount(accountId: string, accountData: CreateAccountData): Promise<Account> {
    try {
      const formattedData = {
        name: accountData.name,
        account_type: accountData.accountType,
      ...
      const response = await apiClient.put(`/api/mongodb/accounts/${accountId}/update/`, formattedData);
      const responseData = response.data as any;
  ...
  async updateDebt(debtId: string, debtData: CreateDebtData): Promise<Debt> {
    try {
      ...
      const response = await apiClient.put(`/api/mongodb/debts/${debtId}/update/`, formattedData);
      const responseData = response.data as any;
