...
import accountsDebtsService, { Account, Debt, CreateAccountData, CreateDebtData } from '../../services/accountsDebtsService';
  ...
  const openAccountDialog = (account?: Account) => {
    if (account) {
      ...
      setAccountForm({
        ...
        effectiveDate: account.effective_date || new Date().toISOString().split('T')[0]
      });
    } else {
      setEditingAccount(null);
      resetAccountForm();
      ...
      <View style={styles.loadingContainer}>
        <Card style={styles.loadingCard}>
          <View style={styles.loadingContent}>
            <Text style={styles.loadingTitle}>Loading your accounts and debts...</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Manage your financial accounts and debts</Text>
      </View>
          ...
          >
            <View style={styles.summaryContent}>
              <View style={styles.summaryItem}>
                ...
                <Text style={styles.summaryValue}>${totalAccountBalance.toLocaleString()}</Text>
              </View>
              <View style={styles.summaryItem}>
                <Ionicons name="card" size={24} color={theme.colors.error} />
                <Text style={styles.summaryLabel}>Total Debts</Text>
                <Text style={styles.summaryValue}>${totalDebtBalance.toLocaleString()}</Text>
              </View>
              <View style={styles.summaryItem}>
                <Ionicons name="trending-up" size={24} color={netWorth >= 0 ? theme.colors.success : theme.colors.error} />
                <Text style={styles.summaryLabel}>Net Worth</Text>
                <Text style={[styles.summaryValue, { color: netWorth >= 0 ? theme.colors.success : theme.colors.error }]}>
      ...
      <View style={styles.tabContainer}>
        ...
        >
          <Ionicons name="wallet" size={20} color={tabValue === 0 ? theme.colors.primary : theme.colors.textSecondary} />
        ...
        >
          <Ionicons name="card" size={20} color={tabValue === 1 ? theme.colors.primary : theme.colors.textSecondary} />
                ...
                <Card key={account.id} style={styles.itemCard}>
                  <View style={styles.itemHeader}>
                    <View style={styles.itemIconContainer}>
                      <Ionicons name={getAccountIcon(account.account_type) as any} size={24} color={theme.colors.primary} />
                    </View>
                    <View style={styles.itemContent}>
                      <Text style={styles.itemTitle}>{account.name}</Text>
                      <Text style={styles.itemSubtitle}>{getAccountTypeLabel(account.account_type)}</Text>
                    </View>
                  ...
                  <View style={styles.itemDetails}>
                    ...
                    <View style={styles.detailRow}>
                      ...
                      <Text style={styles.detailValue}>{account.effective_date}</Text>
                    </View>
                  </View>
                </Card>
              ))
          ...
          <View style={styles.section}>
            ...
            {debts.length === 0 ? (
              <Card style={styles.emptyCard}>
                ...
                <Text style={styles.emptyDescription}>
                  Add your first debt to start tracking your financial obligations
                </Text>
                <Button
              ...
              debts.map((debt) => (
                <Card key={debt.id} style={styles.itemCard}>
                  <View style={styles.itemHeader}>
                    <View style={styles.itemIconContainer}>
                      <Ionicons name={getDebtIcon(debt.debt_type) as any} size={24} color={theme.colors.error} />
                    </View>
                    <View style={styles.itemContent}>
                      <Text style={styles.itemTitle}>{debt.name}</Text>
                      <Text style={styles.itemSubtitle}>{getDebtTypeLabel(debt.debt_type)}</Text>
                    </View>
                    <View style={styles.itemActions}>
                      <TouchableOpacity
                        style={styles.actionButton}
                  ...
                  <View style={styles.itemDetails}>
                    ...
                    <View style={styles.detailRow}>
                      <Text style={styles.detailLabel}>Effective Date</Text>
                      <Text style={styles.detailValue}>{debt.effective_date}</Text>
                    </View>
                    {debt.payoff_date && (
                      <View style={styles.detailRow}>
                        <Text style={styles.detailLabel}>Payoff Date</Text>
                        <Text style={styles.detailValue}>{debt.payoff_date}</Text>
                      </View>
                    )}
                  </View>
                </Card>
        ...
        <View style={styles.modalContainer}>
          ...
          <ScrollView style={styles.modalContent}>
                ...
                <View style={styles.pickerContainer}>
                  {accountTypes.map((type) => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.pickerOption,
                        accountForm.accountType === type.value && styles.pickerOptionSelected
                      ]}
                      onPress={() => {
                        setAccountForm({ 
                          ...accountForm, 
                          accountType: type.value,
                          interestRate: defaultAccountRates[type.value as keyof typeof defaultAccountRates]
                        });
                      }}
                    >
                      <Ionicons 
                        ...
                        size={20} 
                        color={accountForm.accountType === type.value ? theme.colors.primary : theme.colors.textSecondary} 
                      />
                      <Text style={[
                        styles.pickerOptionText,
                        accountForm.accountType === type.value && styles.pickerOptionTextSelected
                      ]}>
                        {type.label}
                      </Text>
                    </TouchableOpacity>
            ...
            <View style={styles.inputRow}>
              <View style={styles.inputHalf}>
                <Input
                  ...
                  onChangeText={(text) => setAccountForm({ ...accountForm, balance: parseFloat(text) || 0 })}
                  placeholder="0.00"
              ...
              <View style={styles.inputHalf}>
                <Input
                  ...
                  onChangeText={(text) => setAccountForm({ ...accountForm, interestRate: parseFloat(text) || 0 })}
                  placeholder="0.00"
                  keyboardType="numeric"
            ...
            <Input
              ...
              onChangeText={(text) => setAccountForm({ ...accountForm, effectiveDate: text })}
                ...
                <View style={styles.pickerContainer}>
                  {debtTypes.map((type) => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.pickerOption,
                        debtForm.debtType === type.value && styles.pickerOptionSelected
                      ]}
                      onPress={() => {
                        setDebtForm({ 
                          ...debtForm, 
                          debtType: type.value,
                          interestRate: defaultDebtRates[type.value as keyof typeof defaultDebtRates]
                        });
                      }}
                    >
                      <Ionicons 
                        name={type.icon as any} 
                        size={20} 
                        color={debtForm.debtType === type.value ? theme.colors.primary : theme.colors.textSecondary} 
                      />
                      <Text style={[
                        styles.pickerOptionText,
                        debtForm.debtType === type.value && styles.pickerOptionTextSelected
                      ]}>
                        {type.label}
                      </Text>
                    </TouchableOpacity>
            ...
            <View style={styles.inputRow}>
              <View style={styles.inputHalf}>
                <Input
                  ...
                  onChangeText={(text) => setDebtForm({ ...debtForm, balance: parseFloat(text) || 0 })}
                  placeholder="0.00"
                  keyboardType="numeric"
                  leftIcon="cash"
                />
              ...
              <View style={styles.inputHalf}>
                <Input
                  ...
                  onChangeText={(text) => setDebtForm({ ...debtForm, interestRate: parseFloat(text) || 0 })}
                  placeholder="0.00"
                  keyboardType="numeric"
                  leftIcon="trending-up"
                />
            ...
            <View style={styles.inputRow}>
              <View style={styles.inputHalf}>
                <Input
                  ...
                  onChangeText={(text) => setDebtForm({ ...debtForm, effectiveDate: text })}
                  placeholder="YYYY-MM-DD"
                  leftIcon="calendar"
                />
              </View>
              <View style={styles.inputHalf}>
                <Input
                  ...
                  onChangeText={(text) => setDebtForm({ ...debtForm, payoffDate: text })}
                  placeholder="YYYY-MM-DD"
                  leftIcon="flag"
                />
              </View>
