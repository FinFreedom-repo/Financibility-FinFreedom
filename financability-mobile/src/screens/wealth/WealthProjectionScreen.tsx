...
const WealthProjectionScreen: React.FC = () => {
  ...
  const [formData, setFormData] = useState<WealthProjectionData>(wealthProjectionService.getDefaultData());
  ...
  const loadSavedSettings = async () => {
    try {
      ...
      if (savedSettings) {
        const sanitizedSettings = wealthProjectionService.sanitizeData(savedSettings);
  ...
  const handleInputChange = (field: keyof WealthProjectionData, value: string) => {
  ...
  const getFormValue = (field: keyof WealthProjectionData): string => {
    ...
    return value !== undefined && value !== null && !isNaN(value) ? value.toString() : '0';
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.assetInterest < 0 || formData.assetInterest > 50) {
      newErrors.assetInterest = 'Asset interest rate must be between 0% and 50%';
    }
    ...
    if (formData.checkingInterest < 0 || formData.checkingInterest > 20) {
      newErrors.checkingInterest = 'Checking interest rate must be between 0% and 20%';
    }
  ...
  const calculateProjection = async () => {
    ...
    try {
      ...
      const response = await wealthProjectionService.calculateWealthProjection(formData);
      
      if (response && response.projections) {
        console.log('💰 Setting projection data:', response.projections.length, 'points');
      ...
      setErrorMessage('Failed to calculate wealth projection. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const importFinancialData = async () => {
    try {
      ...
      if (importedData) {
        const sanitizedData = wealthProjectionService.sanitizeData(importedData);
  ...
  const chartData = useMemo(() => {
    console.log('💰 Preparing chart data, projectionData length:', projectionData?.length);
    ...
    try {
      ...
      console.log('💰 Filtered data length:', filteredData.length, 'Labels length:', labels.length);
      console.log('💰 Chart labels prepared:', labels);
    
    // Calculate Net Worth for each scenario with safety checks
    const netWorthData = filteredData.map(item => (item.scenario_1 || 0) - (item.debt_line || 0));
    const netWorthInflAdjData = filteredData.map(item => (item.scenario_2 || 0) - (item.debt_line || 0));
    const netWorthCheckingData = filteredData.map(item => (item.scenario_3 || 0) - (item.debt_line || 0));
    const netWorthCheckingInflAdjData = filteredData.map(item => (item.scenario_4 || 0) - (item.debt_line || 0));
    ...
    const allDataArrays = [netWorthData, netWorthInflAdjData, netWorthCheckingData, netWorthCheckingInflAdjData, assetsData, debtData];
    
    console.log('💰 Data validation - Labels:', labels.length, 'Filtered data:', dataLength, 'Arrays:', allDataArrays.map(arr => arr.length));
    
    const isValidData = allDataArrays.every(dataArray => dataArray.length === dataLength);
    
    if (!isValidData) {
      console.error('💰 Chart data validation failed: data arrays have different lengths');
      console.error('💰 Expected length:', dataLength, 'Actual lengths:', allDataArrays.map(arr => arr.length));
      return null;
    }
      ...
      <Card style={styles.formCard}>
        ...
        <View style={styles.inputGroup}>
          ...
          <Input
            ...
            onChangeText={(text) => handleInputChange('annualContributions', text)}
            keyboardType="numeric"
      ...
      {showChart && (
        <Card style={styles.chartCard}>
          ...
          {chartData && chartData.labels && chartData.labels.length > 0 ? (
            ...
            <View style={styles.chartContainer}>
              <Text style={styles.placeholderText}>Chart data is being prepared...</Text>
            </View>
          )}
              ...
              <View style={styles.insightsContainer}>
                ...
                <View style={styles.insightItem}>
                  ...
                  <Text style={styles.insightText}>
                    Your wealth could grow by {wealthProjectionService.formatCurrency(
                      (projectionData[projectionData.length - 1].scenario_1 - projectionData[0].scenario_1)
                ...
                <View style={styles.insightItem}>
                  ...
                  <Text style={styles.insightText}>
                    Annual contribution of {wealthProjectionService.formatCurrency(formData.annualContributions || 0)} 
                ...
                {(formData.debt || 0) > 0 && (
                  <View style={styles.insightItem}>
                    <Ionicons name="card" size={16} color={theme.colors.error} />
                    <Text style={styles.insightText}>
                      Debt of {wealthProjectionService.formatCurrency(formData.debt || 0)} 
