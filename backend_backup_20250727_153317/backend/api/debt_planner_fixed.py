...
class DebtPlannerView(APIView):
    ...
    def post(self, request):
        ...
        while any(d['balance'] > 0.01 for d in debts) and month < max_months:
            ...
            if strategy == 'snowball':
                # Snowball: Apply all available extra to the smallest debt first
                for i, d in enumerate(debts):
                    ...
                    if available_extra > 0:
                        ...
                        logger.info(f"{d['name']} - Snowball payment: ${payment:.2f}")
                        available_extra = 0  # Use all available extra on this debt
                        d['balance'] -= payment
                    ...
                    # If we used all extra, add remaining debts without payments
                    if available_extra <= 0:
                ...
                # Avalanche: Apply all available extra to the highest interest rate debt first
                for i, d in enumerate(debts):
                    if d['balance'] <= 0:
                        logger.info(f"{d['name']} is already paid off")
                        month_plan['debts'].append({
                    ...
                    if available_extra > 0:
                        payment = min(available_extra, d['balance'])
                        logger.info(f"{d['name']} - Avalanche payment: ${payment:.2f}")
                        available_extra = 0  # Use all available extra on this debt
                        d['balance'] -= payment
                        d['total_paid'] += payment
